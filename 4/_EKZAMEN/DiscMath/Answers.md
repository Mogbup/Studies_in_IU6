...
51. Метод Квайна–Мак-Класки минимизации булевой функции в классе ДНФ. _(не будет на экзамене)_
...
- *Способы задания графов*
	- 1) Матричный - с помощью матрицы смежности или матрицы инцедентности
		- а) В случае неографа матрица смежности симметрична относительно главной диагонали
		- б) Матрицы смежности изоморфных графов либо совпадают, либо вообще говоря совпадают с точностью до престановки строк и столбцов
		- в) В случае неографа сумма едениц стреугольной матрицы смежности равна числу рёбер графа В случае орграфа сумма единичных элементов равна числу дуг
		- г) Если граф взвешанный, то матрица смежности становится матрицей весов
	- 2) Аналитический - связанс понятием отображенй
		- Прямым отображением вершины Si - Г(si) - назовём множество вершин, смежных  Si, по исходящим дугам.
		- Обратным отображением Г^-1(si) назовём множество вершин, смежных Si, по входящим дугам.
	- 3) Стековый (вектор Айлифа)
	- 4) Массивом пар вершин или массивом рёбер
	
	- *Части графов*
		- 1) Граф G1 называют частью графа G если он находится в отношении включения к G1, т.е. (png/20-1.png)
		- 2) Часть графа называют подграфом, если включение строгое G1cG
		- 3) Часть графа называют сурграфом, если S1=S и U1cU
		
#### <a name="Ответ-21"></a>21. Теоретико-множественные операции на графах.

	Вставить (DiscMath/Материалы высланные Гуренко/Операции на графах.jpg)
	
#### <a name="Ответ-22"></a>22. Маршрут, цепь, цикл, путь, контур в графе. Прямое и обратное транзитивные замыкания.
- Маршрутом, соединяющим S1,S2,...,Sk графа назывется последовательность вершин и рёбер след. вида: S1,U1,S2,U2,...,Sk-1,Uk-1,Sk; в котором два соединённых элемента связанны отношением инцендентности, а через один - отношением смежности относительно элемента между ними.
	- Цепь - маршрут, в котором нет повторяющихся рёбер. Цепь наз. Простой если все её вершины различны.
	- Цикл - простая цепь, начальная вершина которой равна конечной.
	- Контур - цикл в орграфе.
	- Путь - цепь в орграфе.
	
	- Транзитивное замыкание вершины Si - многозначное отбражение вида 
		- Прямое - (png/22-1.png) (до насыщения)
		- Обратное - (png/22-2.png) (до насыщения)
...
- Граф являтеся свзяным тогда и только тогда, когда 2 любые его вершины соеденены маршрутом.

- Точнка сочленения графа - вершина, удаление которой приводит к увеличениею компоненты связности.
	- Если граф имеет хотя бы одну точку сочленения, то такой граф называют разделимым, в противном случае - неразделимым.
	- Теорема: Sk является точкой сочленения связного графа елси и только если существует 2 такие вершины Si и Sj, прчём Si != Sj, что любой путь Si -> Sj проходит через Sk
	- Граф содержащий одну точку сочленения называется односвязным.
	- Граф содержащий i точек сочленения называется i-связынм.
	
#### <a name="Ответ-25"></a>25. Порядковая функция орграфа без контуров. Метод Демукрона отыскания порядковой функции орграфа.
- Порядковой функцией орграфа без контуров называют функцию,задающую отношение порядка на множестве вершин графа, областью значений которой является множество {0,1,2,...,m} , причём (png/25-1.png)
	- Упорядачивает вершины графа лексиграфически, то есть если xi принадлежит Nj, то следующий за ней по пути вершина войдёт в уровень j+1
	
- Метод Демукрона
	- Сложить по вертикали все значения матрицы смежности в каждом столбце
	- Каждая вершина, для которой сумма = 0 принадлежит Ni
	- Вычёркиваем все попавшие в N вершины из матрицы
	- Повторять до тех пор, пока Ai != 0
	- Пример: (png/25-2.png)
...
- Внутренняя устойчивость
	- Подмножество E называется внутренне устойчивым подмножеством вершин графа G если никакие две вершины в E несмежны. Очевидным условие B устойчивого является (png/26-1.png)
	- Подмножетсво E назвыается максимально внутренне устойчивым, если в данном графе не существует такого множетва вершин E', что EcE'
- Внешняя устойчивость
	- Орграф T - внешне устойчивый, если для каждой вершины (png/26-2.png)
	Внешняя устойчивость означает, что любая вершниа не входящаяя в подмножетсво T связана дугой с остальной частю графа, причём начало этой дуги лежит вне T
	- Подмножество T называется минимально внешне устойчивым если в нём не содежится никакого другого внешне устойчивого подмножетво вершин графа G 
	- Для нахождения внутрене устойчивых подмножеств можно использовать фунцию Гранди
		- Функцией Гранди на оргафе G называется функция g(xi), xi принадлежит S, сопоставляющая каждой вершине целове неотрицательное число, наименшее из всех значений, не сопоставленных никакой вершине из Г(xi)
		- Пример: (png/26-3.png)
		
#### <a name="Ответ-27"></a>27. Раскраска графа: постановка задачи, способы решения. Хроматическое число графа. Теорема Кёнига.
- Раскраска графа - присваивание цветов его вершинам/рёбрам так, что смежные вершины/рёбра не окрашены в один цвет.
- Если для решения задачи применено r цветов, то граф назывют r-хроматическим
- Наименьшее число r назыают хроматическим числом графа x(G) (там не x а кси)
- Множество вершин одного цвета называют одноцветным классом
	
- Теорема: Для того, что бы неограф G имел r = x(G) необходимо и достаточно, что бы существовал соответсвующий ему симметрический граф без петель, который допускал бы такую функцию Гранди, что max g(xi) =< x(G) - 1; xi принадлежит S
...
- Клика - полный подграф заданного неографа  (Граф, в котором любая вершина смежна с любой другой - полный)
- Клика, которая не содержит никакой другой клии называется максимальной
- Клика с наибольшим числом вершин называется наибольшей.
	- Число вершин наибольшей клики называется кликовым числом. Обозначение - ф(G) (фи)
	
- Нахождение максимальной(наибольшей) клики негорафа можно применить следующий алгоритм
	- Построить граф G
	- Сопоставить этому графу некоторый орграф
	- В полученном орграфе определить максимальные внутрене устойчивые подмножества вершин S1,S2,...,Sm
	- Отобразить полученные S1,S2,...,Sm на исходный граф G
	- Вершины, пренадлежищие Si и соедияющие их рёбра исходного графа G обзауют i-ю максимальную клику.
...
- Эйлеровым циклом(обходом) в неографе называется цикл, включающий все рёбра графа и проходящий по каждому ребру в точности один раз. Граф, содержащий эйлеровский цикл называют эйлеровским.
	-Теорема "О эйлеровых циклах": Связный неограф является эйлеровским тогда и только тогда, когда все его вершины имеют чётную степень.
		- Доказательство:
			- Необходимость: Пусть связаный неограф G эйлеров. => цикл в G проходит через каждую вершину. Для каждой вершины верно: шикл входит в вершину по одному ребру, а выходит по другому. Таким образом, каждая вершина инцидентна чётному числу рёбер. Така как эйлеров цикл по определению содержит все рёбра графа, то степени всех вершин чётны.
			- Достаточность: *Я не переписываю это всё* (DiscMath/Материалы высланные Гуренко/Th об эйлеровых циклах.jpg)
	- Существет алгоритм Флёри построения эйлерова цикла
		- Выбираем S1 и инцедентное ей ребро U1
		- По U1 переходим в S2
		- Для S2 повторяем п.1
		- Алгоритм проделывать до тех пор, пока не будут помечены все рёбра графа. При этом находять в S1 не следует
			- Выбирать ребро, инцендетное Si и S1, если есть другие варианты 
			- Выбирать ребро-перешеек - ребро, уделние которого разделяет граф на 2.
			
#### <a name="Ответ-30"></a>30. Гамильтоновы графы. Классические задачи о гамильтоновом цикле. Теорема Оре о гамильтоновом графе.

- Неограф называется Гамильтоновым, если он связный и содержит Гамельтонов цикл.
- Гамельтонов цикл - цикл, содержащий все вершины графа и проходящий по каждой вершине ровно один раз.

- Теорема Оре: Дан связный неограф порядка N>=3. Если для любой пары не смежных вершин Si Sj выполняется условие (png/30-1.png), то граф - гамильтонов.
...
#### <a name="Ответ-51"></a>52. Классы Поста булевых функций: сохраняющих константу нуля и константу единицы, линейных и монотонных.
#### <a name="Ответ-52"></a>53. Двойственность булевых функций. Способ отыскания функции, двойственной к заданной. Теоремы о двойственности. Класс Поста самодвойственных функций.
#### <a name="Ответ-53"></a>54. Замкнутый класс. Полные системы булевых функций. Теорема Поста. Примеры полных систем булевых функций.
#### <a name="Ответ-54"></a>55. Порядок доказательства полноты произвольной системы булевых функций.
